...
const { createInitialLead } = require('./_lead-sync');
const { inferVariant, getClientIp } = require('./_meta');
...
module.exports = async function createPayment(req, res) {
  ...
  const payment = {
    ...
    returnUrl:
      `${siteUrl}/api/payment-return?order=${encodeURIComponent(orderReference)}` +
      `&token=${encodeURIComponent(statusToken)}`,
    serviceUrl: `${siteUrl}/api/wayforpay-callback`
  };

  const host = req.headers['x-forwarded-host'] || req.headers.host || merchantDomainName;
  const landingVariant = inferVariant(host);

  const leadSync = await createInitialLead({
    orderReference,
    fullName,
    phone,
    email,
    paymentStatus: 'Без спроби оплати',
    transactionStatus: '',
    amount,
    currency: CURRENCY,
    product: PRODUCT_NAME,
    site: merchantDomainName,
    ...
    source_url: body.source_url || siteUrl,
    fbp: body.fbp || '',
    fbc: body.fbc || '',
    ip: getClientIp(req),
  ...
  if (!leadSync || !leadSync.ok) {
    console.error('Initial lead sync failed:', leadSync);
    return res.status(502).json({ message: 'Не вдалося зберегти контактні дані. Спробуйте ще раз.' });
  }

  res.setHeader('Cache-Control', 'no-store');
  return res.status(200).json({ payment, orderReference, amount, statusToken });
};
